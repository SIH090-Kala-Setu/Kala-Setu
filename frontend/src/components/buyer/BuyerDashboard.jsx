...
export default function BuyerDashboard() {
  ...
  const filteredInquiries = (data.inquiries || []).filter(i => filter === 'All' || i.status === filter);
    ...
    <div className="container" style={{ padding: '24px 0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
        <div className="card" style={{ padding: '20px', borderLeft: '4px solid #3498DB' }}>
          ...
          <h3 style={{ margin: '8px 0 4px' }}>{data.total_inquiries}</h3>
        ...
        <div className="card" style={{ padding: '20px', borderLeft: '4px solid #F39C12' }}>
          ...
          <h3 style={{ margin: '8px 0 4px' }}>{data.pending_inquiries}</h3>
        ...
        <div className="card" style={{ padding: '20px', borderLeft: '4px solid #2ECC71' }}>
          ...
          <h3 style={{ margin: '8px 0 4px' }}>{data.completed_inquiries}</h3>
          ...
          <tbody>
            {filteredInquiries.map(i => (
              <tr key={i.id}>
                ...
                <td style={{ fontWeight: 500 }}>{i.product_title}</td>
                <td>{i.artisan_name}</td>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
        {data.suggested_artisans?.map(a => (
          <div key={a.id} className="card" style={{ padding: '16px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', marginTop: '4px' }}>📍 {a.location}</p>
          </div>
        ))}
